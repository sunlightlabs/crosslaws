	...
	for element in doc.xpath('//div[@class="yearmaster"]'):   #Could also use "statutesatlargevolumemasterhead"
		for d_element in element.xpath('span'):
			...
			for m_element in d_element.xpath('a'):
				...
				#print text, url
				#releases += [(text, url)]
				subsubreleases += add_subsubrelease(url)
...
dataset += add_release("http://uscode.house.gov/table3/table3years.htm") #Could also use "/alltable3statutesatlargevolumes.html"
